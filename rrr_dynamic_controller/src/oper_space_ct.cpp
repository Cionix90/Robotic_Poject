#include <rrr_dynamic_controller/oper_space_ct.hpp>
#include <math.h>
...
namespace rrr_dynamic_controller{
    Operational_Space_CT::Operational_Space_CT(){}
    Operational_Space_CT::~Operational_Space_CT(){}

    void Operational_Space_CT::check_set_EE_setpoint()
    {
        ...
        if(check_frame.p(0) == cmd_EE_state(0) && check_frame.p(1) == cmd_EE_state(1) && check_frame.p(2) == cmd_EE_state(2))
        {
            new_cmd = false;
            time_cmd = 0.0;
    ...
    bool Operational_Space_CT::advance_step(double time, KDL::JntArray& advance)
    {
    ...
    bool Operational_Space_CT::compute_xi_(KDL::JntArray advance, KDL::JntArrayAcc& des_traj)
    {
    ...
    void Operational_Space_CT::get_Param(ros::NodeHandle& n,std::string name, std::string& param)
    {
    ...
    bool Operational_Space_CT::init(hardware_interface::EffortJointInterface* hw,ros::NodeHandle& n)
    {
        ...
        Operational_Space_CT::get_Param(n,"urdf", urdf);
        Operational_Space_CT::get_Param(n,"first_link", base_link);
        Operational_Space_CT::get_Param(n,"ee_link", ee_link);
       ...
       /* J_inv_.resize(n_joint);
        ...
        cmd_sub = n.subscribe(
            "command",
            1,
            &Operational_Space_CT::command,
            this
        );
        st_gain_sub = n.subscribe(
            "set_gains",
            1,
            &Operational_Space_CT::set_gain,
            this
        );
    ...
    void Operational_Space_CT::update(const ros::Time& time, const ros::Duration& period)
    {
        int res;
        ...
        if(new_cmd)
        {
            ...
            */
            ...
            //ROS_INFO("%lf,il tempo attuale vale %lf",period.toSec(),time_cmd);
    ...
    void Operational_Space_CT::command(const std_msgs::Float64MultiArray::ConstPtr& msg)
    {
    ...
    void Operational_Space_CT::set_gain(const std_msgs::Float64MultiArray::ConstPtr& msg)
    {
    ...
    void Operational_Space_CT::starting(const ros::Time& time)
    {
        int res;
        for(unsigned int i=0;i<n_joint;i++)
        {
    ...
    void Operational_Space_CT::stopping(const ros::Time& time)
    {}

PLUGINLIB_EXPORT_CLASS(rrr_dynamic_controller::Operational_Space_CT, controller_interface::ControllerBase);
}
